test_that("new_tripo_task creates correct class", {
  task <- new_tripo_task(
    task_id = "task-123",
    status = "success",
...
test_that("new_tripo_task handles running status", {
  task <- new_tripo_task(
    task_id = "task-456",
    status = "running",
    created_at = "2024-01-01T00:00:00Z",
...
test_that("new_tripo_task handles error status", {
  task <- new_tripo_task(
    task_id = "task-fail",
    status = "error",
    created_at = "2024-01-01T00:00:00Z",
...
test_that("print.tripo_task does not error", {
  task <- new_tripo_task("task-1", "success", Sys.time(),
                         model_url = "https://api.tripo3d.ai/models/model.glb",
                         error_message = NULL, raw = list())
  expect_no_error(print(task))
})

test_that("new_tripo_model creates correct class", {
...
test_that("print.tripo_model does not error", {
  model <- new_tripo_model("/tmp/test.glb", "task-1",
